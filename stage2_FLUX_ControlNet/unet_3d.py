...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def __init__(
        ...
        in_channels_image: int = 4,
        in_channels_mask: int = 4,
        out_channels: int = 1,
        ...
        block_out_channels: Tuple[int, ...] = (128, 256, 256, 320),
        ...
        self.conv_in = nn.Conv2d(
            in_channels_mask, block_out_channels[0]//2, kernel_size=conv_in_kernel, padding=conv_in_padding
        )
        self.conv_img_in= nn.Conv2d(
            in_channels_image, block_out_channels[0]//2, kernel_size=conv_in_kernel, padding=conv_in_padding
        )
    ...
    def forward(
        ...
        if any(s % default_overall_up_factor != 0 for s in sample_mask.shape[-2:]):
        ...
        num_frames = sample_mask.shape[2]
        timesteps = timesteps.expand(sample_mask.shape[0])
        ...
        # 2. pre-process
        sample_mask = sample_mask.permute(0, 2, 1, 3, 4).reshape((sample_mask.shape[0] * num_frames, -1) + sample_mask.shape[3:])
        sample_mask = self.conv_in(sample_mask)
        sample_image = self.conv_img_in(sample_image)
        sample_image = sample_image.repeat_interleave(sample_mask.shape[0], dim=0)
    
        sample = torch.cat([sample_mask,sample_image],dim=1)
