...
class TestMLLMTokenizeFn(TestCase):
    ...
    def test_intern_vl_single_image(self):
        ...
        with open(data_path) as f:
            for i, line in enumerate(f):
                ...
                # Remove \n because the InternVL chat_template adds it.
    ...
    def test_intern_vl_multi_image(self):
        ...
        total_step = 5
        # input_ids cannot align by design because the HF implementation is incorrect. For multiple images, max_num should shrink, but HF processes images independently, so the number of <IMG_CONTEXT> tokens will not match.
        with open(data_path) as f:
            for i, line in enumerate(f):
                if i >= total_step:
                    break
