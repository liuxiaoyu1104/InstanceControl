...
class TrainingController:
    ...
    def _grouped_by_max_length(self, packed_data_batches):
        # After sorting, the first batch may contain many pad tokens because the last pack may contain only a few real samples.
        # For example, if 16 packs are formed, the 16th pack may contain only a few real samples and the rest are pad tokens.
        # After sorting, this pack is placed first, so rank 0 often consumes fewer valid tokens than other ranks in the first step; this is expected.
