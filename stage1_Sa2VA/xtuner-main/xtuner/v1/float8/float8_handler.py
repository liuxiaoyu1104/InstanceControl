...
# Should each handler correspond one-to-one with an Engine?
class Float8Handler:
    ...
    def get_num_features_after_pad(tensor_size, fsdp_shard_dim, num_chunks, fp8_block_size=128):
        ...
        if total_size < fp8_block_size:
            # For small tensors, pad to fp8_block_size; this is rare in practice.
            total_size = fp8_block_size
        ideal_chunk_size = math.ceil(total_size / num_chunks)
        if ideal_chunk_size > fp8_block_size:
            # If greater than base_size, only allow n * 128 or n * 128 + 64; the extra 64 simplifies reduction.
            ...
            # If smaller than base_size, find a factor of 128 that is greater than or equal to ideal_chunk_size.
    ...
    def _build_reduce_mesh_devided_64(self, fsdp_mesh: DeviceMesh):
        # Support the case where MoE parameters are split by FSDP and EP into dout = n * 128 + 64 (n >= 1).
        # The last 64 dims of FSDP rank 0 and the first 64 dims of FSDP rank 1 form one block together.
        # Use reduce max when computing absmax.
        if not self.enabled:
            logger.warning("Float8 training is not enabled.")
            return
    ...
    def _build_reduce_mesh_mapping(self, model: nn.Module, fsdp_mesh: DeviceMesh):
        ...
        for module in model.modules():
            if isinstance(module, (TileWiseFloat8Linear, TileWiseFloat8GroupedLinear)):
                ...
                # Local shapes are identical across ranks because padding was already applied in convert_to_float8_training.
