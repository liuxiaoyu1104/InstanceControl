...
def precompute_tilewise_float8_scale_for_fsdp(
    ...
    for weight in weights:
        # Local tensor shapes are identical across ranks because padding was applied.
    ...
    for local_shape, weights_same_shape in shape_grouped_weights.items():
        ...
        for i, w in enumerate(weights_same_shape):
            # torch compile may produce incorrect results when handling tensors whose storage_offset is not 0.
            # See https://github.com/pytorch/pytorch/issues/155690.
            # Although cloning adds a small overhead of a few milliseconds, it ensures correctness of later results.
...
def cast_to_per_block_fp8_devided_64_with_scales(
    ...
    if rank % 2 == 0:
        # The last 64 dims belong to one block.
        tensor0 = tensor[:dout_0]
        tensor1 = tensor[dout_0:]
    else:
        # The first 64 dims belong to one block.
        tensor0 = tensor[64:]
        tensor1 = tensor[:64]
    tensor0 = (
    ...
    def fsdp_post_all_gather(
        ...
        *,
        ...
        if (self._tensor.shape[0] >= 128) and (self._tensor.shape[0] % 128 == 64):
            ...
            # Slice off the scales corresponding to the two duplicated 64-dim chunks.
            scale0 = scale[:, 0]
            scale1 = scale[:, 1]

            # The resulting scales are based on the fsdp_mesh all_gather result. The last 64 dims of FSDP rank 0
            # and the scales for the first 64 dims of rank 1 should theoretically be identical, unless reduce max introduces numerical differences.
            ...
            # Since dim0 has been padded to a multiple of 128, dim0 // 128 can be used directly here.
            # dim1 is also asserted to be a multiple of 128 in from_float.
            # reduce max was already applied when computing amax, and all_gather produced duplicated scales across ranks.
            # Slice them off.
            scale = scale.view(dim0 // 128, -1, dim1 // 128)
            scale = scale[:, 0]
        scale = scale.contiguous()
        # Padded weights and scales are handled in
        # _xtuner/xtuner/_lite/accelerate/float8_gmm/float8_gmm_tile_wise.py and
        # _xtuner/xtuner/_lite/accelerate/float8_gmm/float8_linear_tile_wise.py
        # by slice_weight autograd.
