...
class MoE(BaseModel):
    ...
    def __init__(self, config: MoEConfig):
        ...
        # TODO(@yehaochen): Remove these two lines. _maybe_compile_layers should keep compile-related settings outside fsdp_config.
        # Move _init_load_spec into post init.
    ...
    def update_bias(self, total_expert_counts_pre_iter, expected_loads):
        ...
        for i_layer in range(n_layer):
            # The first l layers are MLP layers; skip them.
            ...
            update_mask = load_diff != 0  # Update only the experts that need adjustment.
    ...
    def build_layers(self, config: MoEConfig) -> nn.ModuleDict:
        # Make layers an nn.ModuleDict to simplify parameter splitting for pipeline parallelism,
        # so idx remains unchanged after some layers are removed.
    ...
    def fully_shard(
        ...
        # TODO: This is required because model initialization builds an ep_mesh; without rebuilding it, fsdp_mesh and ep_mesh will be unrelated.
        # fully_shard would otherwise raise: AssertionError: FSDP requires the DP and TP mesh to have the same parent mesh.
