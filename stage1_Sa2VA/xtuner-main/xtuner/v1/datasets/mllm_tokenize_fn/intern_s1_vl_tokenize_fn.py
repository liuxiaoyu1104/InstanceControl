...
class InternS1VLTokenizeFunction(BaseMLLMTokenizeFunction[InternS1DataItem]):
    def __init__(
        ...
        # Note: This is important to prevent changing parameters without rebuilding the cache.
        self._hash_str = (
    ...
    def calc_num_tokens_video_get_item(self, data_item) -> CacheItem:
        # TODO: Currently only one video is supported.
        # Generate a deterministic random integer from data_item.
        random_frame_num = generate_random_int_from_dict(data_item, self.min_num_frames, self.max_num_frames)
        # Compute the token count from the sampled frame count (min_num_frames, max_num_frames + 1). The actual video may not contain that many frames, so num_tokens may be overestimated.
