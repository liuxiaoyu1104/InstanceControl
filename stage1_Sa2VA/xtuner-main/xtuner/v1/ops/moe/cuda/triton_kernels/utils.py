...
def generate_random_list(length, total_sum):
    # Generate a list of length `length` whose elements sum to total_sum.
    # First generate an evenly distributed list.
    avg = total_sum // length
    lst = [0] * length
    # Randomly adjust values while keeping the sum unchanged.
    for i in range(length):
        # Randomly choose two different positions.
