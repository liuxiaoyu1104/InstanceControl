...
class SingleAcceleratorWorker:
    ...
    def setup_distributed(self, rank: int, master_addr: str, master_port: int, world_size: int):
        ...
        # The backend parameter specifies the communication backend; it is not read from environment variables.
        # - 'nccl': communication between NVIDIA GPUs, recommended for GPU.
        # - 'gloo': CPU communication or cross-platform communication.
        # - 'mpi': requires an MPI environment.
        if self.accelerator == "GPU":
            backend = "nccl"
        elif self.accelerator == "NPU":
            backend = "hccl"
        ...
        dist.init_process_group(
            backend=backend,
            init_method="env://",  # This tells PyTorch to read configuration from environment variables.
        )
