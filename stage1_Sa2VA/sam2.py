...
class SAM2(nn.Module):
    ...
    def language_embd_inference(self, inference_state, language_embd):
        ...
        mask_out = []
        iou_out =[]
        object_score_logit_out= []
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            for frame_idx in range(num_frame):
                frame_mask_out = []
                frame_iou_out =[]
                frame_object_score_logit_out= []

                for obj_idx in range(num_obj):
                    _language_embd = language_embd[frame_idx][obj_idx][None]
                    # print(_language_embd.shape)
                    _, _, out_mask_logits,ious,object_score_logits = self.sam2_model.add_language_embd(
                        inference_state,
                        frame_idx,
                        obj_idx + 100,
                        _language_embd,
                    ...
                    frame_mask_out.append(out_mask_logits)
                    frame_iou_out.append(ious.unsqueeze(0))
                    frame_object_score_logit_out.append(object_score_logits.unsqueeze(0))
                frame_mask_out = torch.cat(frame_mask_out, dim=1)
                frame_iou_out = torch.cat(frame_iou_out, dim=1)
                frame_object_score_logit_out = torch.cat(frame_object_score_logit_out, dim=1)
                # print(frame_mask_out.shape, frame_iou_out.shape, frame_object_score_logit_out.shape)
                mask_out.append(frame_mask_out)
                iou_out.append(frame_iou_out)
                object_score_logit_out.append(frame_object_score_logit_out)

            # mask_out = []
            # iou_out = []
            # object_score_logit_out = []
            # for out_frame_idx, out_obj_ids, out_mask_logits,ious,object_score_logits in self.sam2_model.propagate_in_video(inference_state):
            #     mask_out.append(out_mask_logits)
            #     iou_out.append(ious)
            #     object_score_logit_out.append(object_score_logits)
            # mask_out = torch.cat(mask_out, dim=0)
            # iou_out = torch.cat(iou_out, dim=0)
            # object_score_logit_out = torch.cat(object_score_logit_out, dim=0)
            
            mask_out = torch.cat(mask_out, dim=0)
            iou_out = torch.cat(iou_out, dim=0)
            object_score_logit_out = torch.cat(object_score_logit_out, dim=0)

        return mask_out,iou_out,object_score_logit_out
...
class SAM2Base_(torch.nn.Module):
    ...
    def _forward_sam_heads(
        ...
        if multimask_output:
            ...
            iou_best = ious[batch_inds, best_iou_inds].unsqueeze(0)
            # iou_best = ious
            
            low_res_masks = low_res_multimasks[batch_inds, best_iou_inds].unsqueeze(0)
            high_res_masks = high_res_multimasks[batch_inds, best_iou_inds].unsqueeze(0)
...
class SAM2Base(SAM2Base_):
    ...
    def _forward_sam_heads(
        ...
        if multimask_output:
            ...
            batch_inds = torch.arange(B, device=device)
            iou_best = ious[batch_inds, best_iou_inds].unsqueeze(1)
            # iou_best = ious
...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_language_embd(
        ...
        if inference:
            ...
            # )
        return frame_idx, obj_ids, pred_mask_gpu,current_out['ious'],current_out['object_score_logits']
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video", disable=True):
            ...
            yield frame_idx, obj_ids, video_res_masks,ious,object_score_logits
