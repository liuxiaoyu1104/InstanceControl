...
class VideoLLaVASAMModel(LisaModel):
    def __init__(self,
        ...
        # if not self.mllm.freeze_visual_encoder:
        #     self.mllm.model.vision_model.requires_grad_(True)
        ...
        pretrained_pth = './pretrained/Sa2VA-4B'
        if pretrained_pth is not None:
            ...
            for file in files:
                load_files_dict = load_file(os.path.join('./pretrained/Sa2VA-4B', file))
                merge_state_dict.update(load_files_dict)

            new_state_dict = OrderedDict()
            for key, value in merge_state_dict.items():
                if key.startswith('vision_model'):
                    new_key = key.replace('vision_model','mllm.model.vision_model')
                    new_state_dict[new_key] = value
                elif key.startswith('language_model.model'):
                    new_key = key.replace('language_model.model', 'mllm.model.language_model.base_model.model.model')
                    if 'self_attn' in key or 'mlp.down_proj' in key or 'mlp.gate_proj' in key or 'mlp.up_proj' in key:
                        new_key = new_key.replace('weight', 'base_layer.weight')
                        new_key = new_key.replace('bias', 'base_layer.bias')
                    new_state_dict[new_key] = value
                elif key.startswith('language_model.lm_head'):
                    new_key = key.replace('language_model.lm_head', 'mllm.model.language_model.base_model.model.lm_head')
                    new_state_dict[new_key] = value
                elif key.startswith('mlp1'):
                    new_key = key.replace('mlp1', 'mllm.model.mlp1')
                    new_state_dict[new_key] = value
                elif 'g_weight' in key:
                    new_key = key.replace('g_weight', 'gamma')
                    new_state_dict[new_key] = value
                else:
    ...
    def state_dict(self, *args, **kwargs):
        ...
        if not self.frozen_sam2_decoder:
            to_return.update(
                {k: v
    ...
    def check_obj_number(self, pred_embeddings_list_video, gt_masks_video,seg_ids, fix_number=5):
        assert len(pred_embeddings_list_video) == len(gt_masks_video)
        ret_pred_embeddings_list_video = []
        ret_gt_masks_video = []
    
        for pred_mebeds, gt_masks,seg_id in zip(pred_embeddings_list_video, gt_masks_video,seg_ids):
            # assert len(pred_mebeds) == len(gt_masks)
            count =0 
            if len(pred_mebeds) != len(gt_masks):
                min_num = min(len(pred_mebeds), len(gt_masks))
            ...
            if len(set(seg_id)) != fix_number or len(set(seg_id)) == fix_number:
                if len(set(seg_id)) >= fix_number:
                    object_idxs = torch.randperm(max(seg_id))+1
                    object_idxs = object_idxs[:fix_number]

                    pred_mebeds_list =[]
                    gt_masks_list =[]
                    for object_idx in object_idxs:
                        _idxs = [i for i, seg in enumerate(seg_id) if seg == object_idx]
                        count +=len(_idxs)
                        pred_mebed = pred_mebeds[_idxs]
                        gt_mask = gt_masks[_idxs[0:1]]
                        pred_mebeds_list.append(pred_mebed)
                        gt_masks_list.append(gt_mask)
                    gt_masks_list = torch.cat(gt_masks_list,dim=0)
                else:
                    n_repeat = fix_number // len(set(seg_id)) + 1
                    object_idxs = torch.randperm(max(seg_id))+1
                    object_idxs = torch.cat([object_idxs] * n_repeat, dim=0)[:fix_number]
                    ...
                    # pred_mebeds = torch.cat([pred_mebeds] * n_repeat, dim=0)[:fix_number]
                    # gt_masks = torch.cat([gt_masks] * n_repeat, dim=0)[:fix_number]
            ret_pred_embeddings_list_video.append(pred_mebeds_list)
            ret_gt_masks_video.append(gt_masks_list)
        return ret_pred_embeddings_list_video, ret_gt_masks_video
    ...
    def forward(self, data, data_samples=None, mode='loss'):
        ...
        language_embeddings =[]
        for pred_embeddings_video in pred_embeddings_list_video:
            language_embeddings = language_embeddings + pred_embeddings_video
        for i in range(language_embeddings):
            language_embeddings[i] = language_embeddings[i][:, None]
    ...
    def sample_points(self, mask_pred, gt_masks):
        # print("-------------------")
        # print(gt_masks.shape,gt_masks.max(),gt_masks.min())
        # print(mask_pred.shape,mask_pred.min(),mask_pred.max())
...
class VideoLLaVASAMModel_zero3(VideoLLaVASAMModel):
    ...
    def _parse_seg_ids_from_input(self, input_ids):
        """
        Parse the ID corresponding to each SEG token from input_ids.
        For example, [SEG] <1> [SEG] <1> [SEG] <2> should return [1, 1, 2].
        """
        batch_size = input_ids.shape[0]
        seg_ids_list = []
        
        for i in range(batch_size):
            # Get the token sequence for the current batch item.
            tokens = input_ids[i]
            tokens_text = self.tokenizer.decode(tokens, skip_special_tokens=False)
            pattern = r'\[SEG\]\s*<(\d+)>'
            matches = re.findall(pattern, tokens_text)
            numbers = [int(match) for match in matches]
            seg_ids_list.append(numbers)
            
        return seg_ids_list

    def forward(self, data, data_samples=None, mode='loss'):
        
        # Print all trainable parameters of self.mllm.
        # print("Self.mllm parameters:")
        # for name, param in self.mllm.named_parameters():
        #     if param.requires_grad and "vision" in name:  # Only print parameters that require gradient updates.
        #         print(f"{name}: {param.shape}")

        # # Print all trainable parameters of self.grounding_encoder.
        # print("\nSelf.grounding_encoder parameters:")
        # for name, param in self.grounding_encoder.named_parameters():
        #     if param.requires_grad:  # Only print parameters that require gradient updates.
        #         print(f"{name}: {param.shape}")
        ...
        _zero = hidden_states.mean() * 0.0
        if seg_valid:
            pred_embeddings = hidden_states[:,:-1,:][seg_token_mask] + _zero
            seg_ids = self._parse_seg_ids_from_input(input_ids)
        ...
        gt_masks_video = self.process_video_gt_masks(gt_masks, frames_per_batch)
        # pred_embeddings_list_video, gt_masks_video = self.check_obj_number(
        #     pred_embeddings_list_video, gt_masks_video,seg_ids
        # )
        pred_embeddings_list_video, gt_masks_video = self.check_obj_number(
            pred_embeddings_list_video, gt_masks_video,seg_ids
        )
        ...
        for pred_embeddings_video in pred_embeddings_list_video:
            language_embeddings = language_embeddings + pred_embeddings_video
        for i in range(len(language_embeddings)):
            language_embeddings[i] = language_embeddings[i][ None,:]

        sam_states = self.grounding_encoder.get_sam2_embeddings(g_pixel_values, expand_size=num_objs)
        # pred_masks = self.grounding_encoder.inject_language_embd(sam_states, pred_embeddings_list_video, seg_ids_obj, nf_nobj=(num_frames, num_objs))
        pred_masks = self.grounding_encoder.inject_language_embd(sam_states, language_embeddings, nf_nobj=(num_frames, num_objs))
        # print(gt_masks_video[0].shape)
        gt_masks = [F.interpolate(gt_mask.unsqueeze(0), size=pred_masks[0].shape[-2:], mode='nearest').squeeze(0) for gt_mask in gt_masks_video]
        gt_masks = torch.cat(gt_masks, dim=0)
        pred_masks = pred_masks.flatten(0, 1)
        ...
        if self.loss_sample_points:
            sampled_pred_mask, sampled_gt_mask = self.sample_points(pred_masks, gt_masks)
            # print("++++++++++++")
            # print(sampled_pred_mask.shape, sampled_gt_mask.shape, (len(gt_masks) + 1e-4))
            # print(sampled_pred_mask.shape, sampled_gt_mask.shape, (pred_masks.shape[0] * sampled_pred_mask.shape[1] + 1e-4))
            sam_loss_dice = self.loss_dice(
                sampled_pred_mask,
                sampled_gt_mask, avg_factor=(len(gt_masks) + 1e-4))
            sam_loss_mask = self.loss_mask(
                ...
                sampled_gt_mask.reshape(-1),
                avg_factor=(pred_masks.shape[0] * sampled_pred_mask.shape[1] + 1e-4))
            # print(sam_loss_dice, sam_loss_mask)
        else:
            sam_loss_mask = self.loss_mask(pred_masks, gt_masks)
            sam_loss_dice = self.loss_dice(pred_masks, gt_masks)
        loss_mask += sam_loss_mask
        ...
        mean_loss = (loss_mask + loss_dice + output.loss) / 3
