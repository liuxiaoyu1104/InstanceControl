...
class SAM2TrainRunner(BaseModule):
    ...
    def inject_language_embd(self, sam_states, language_embd, nf_nobj=None):
        ...
        low_res_mask_list =[]
        B_all = sam_states['current_vision_feats'][-1].size(1)  # batch size on this frame
        for batch_idx in range(B_all):
            C = self.hidden_dim
            B= sam_states['current_vision_feats'][-1][:,batch_idx:(batch_idx+1),:].size(1)
            H, W = sam_states['feat_sizes'][-1]

            if self.sam2_model.directly_add_no_mem_embed:
                # directly add no-mem embedding (instead of using the transformer encoder)
                pix_feat_with_mem = sam_states['current_vision_feats'][-1][:,batch_idx:(batch_idx+1),:] + self.sam2_model.no_mem_embed
            ...
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
                ...
                _, _, _, low_res_mask, _, _, _, = self.sam2_model._forward_sam_heads(
                    backbone_features=pix_feat_with_mem,
                    point_inputs=None,
                    mask_inputs=None,
                    high_res_features=[high_res_features[0][batch_idx:(batch_idx+1),:],high_res_features[1][batch_idx:(batch_idx+1),:]],
                    multimask_output=self.sam2_model._use_multimask(is_init_cond_frame=True, point_inputs=None),
                    # Inject language Embed if possible
                    language_embd=language_embd[batch_idx],
                )
                low_res_mask_list.append(low_res_mask)
