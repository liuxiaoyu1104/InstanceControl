...
GCG_QUESTIONS = [
    DEFAULT_IMAGE_TOKEN + 'Please identify each object described in {prompt}, and provide the rewirte description along with its corresponding interleaved segmentation mask in image.',
    ...
    DEFAULT_IMAGE_TOKEN + 'Using both the provided image and the prompt in {prompt}, locate each mentioned object, then rewirte the prompt with an interleaved segmentation mask for every instance',
    DEFAULT_IMAGE_TOKEN + 'Based on the image and the description in {prompt}, identify all described objects and rewirte the description along with its corresponding interleaved segmentation mask for each one',
    DEFAULT_IMAGE_TOKEN + 'Analyze the image along with the description in {prompt}, extract each object, and rewirte the description along with a corresponding segmentation mask',
...
def refcocog_parse_annotations(example):
    ...
    annotations = {'labels': [],'labels_idx':[], 'caption': [],'caption_usr': [], 'masks': [], 'tokens_positive': [],
                   'file_name': example['canny_img_file'], 'image': example['canny_img_file']}
    ...
    for detail in example['refs']:
        ...
        phrase_idx = detail['Instance ID']
        if phrase.lower() in annotations['caption']:
            annotations['labels'].append(phrase)
            annotations['labels_idx'].append(phrase_idx)
            index = annotations['caption'].lower().find(phrase.lower(),search_start)
            if index ==-1:
                index = annotations['caption'].lower().find(phrase.lower()[:50],search_start)
                if index ==-1:
                    index = annotations['caption'].lower().find(phrase.lower()[:20],search_start)
                if index ==-1:
                    print(example['canny_img_file'])
                    print("============"+phrase.lower()[:20])
            end_index = index + len(phrase) if index != -1 else -1
            search_start = max(end_index,search_start)
    ...
    annotations['labels_idx'] = [annotations['labels_idx'][i] for i in sorted_indices]
...
def refcocog_conversation(caption, tokens_positive,labels_idx):
    # insert <p> </p> and [seg] to caption and select a question
    question = random.choice(GCG_QUESTIONS).format(prompt=caption).strip()

    # Prepare caption with tags
    def tag_caption(caption, tokens,labels_idx):
        ...
        for (start, end), label_id in sorted(zip(tokens, labels_idx), key=lambda x: x[0][0], reverse=True):
            # print("=======")
            # print(label_id)
            caption = f"{caption[:start]}<p> {caption[start:end]} </p> [SEG] <{label_id}>{caption[end:]}"
        return caption

    detailed_answer = tag_caption(caption.strip(), tokens_positive,labels_idx)
...
def refcocog_preprocess(example):
    ...
    labels_idx = sort_by_start_index(labels_idx, phrase_order)
    tokens_positive = sort_by_start_index(tokens_positive, phrase_order)

    conversations = refcocog_conversation(caption,tokens_positive,labels_idx)
    example['conversations'] = conversations
    example['labels'] = data_labels
    example['masks'] = masks
    example['tokens_positive'] = tokens_positive
    example['labels_idx'] = labels_idx
