...
class RefCOCOgGCGDataset(GCGDataset):
    ...
    def decode_mask(self, object_masks, ori_height, ori_width):
        ...
        masks = np.stack(binary_masks, axis=0)
        masks = torch.from_numpy(masks)
        if masks.shape[1:] != (ori_height, ori_width):
            # Add a channel dimension and resize.
            masks = masks.unsqueeze(1).float()  # Convert to float for interpolation.
            masks = F.interpolate(masks, size=(ori_height, ori_width), mode='nearest')
            masks[masks>=0.5]=1.0
            masks[masks<0.5]=0.0
            masks = masks.squeeze(1).byte()  # Remove the channel dimension and convert back to byte/uint8.
        # print(masks.shape)
        return masks
