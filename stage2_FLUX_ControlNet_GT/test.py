...
def get_mask_inputs(json_path, tokenizer, mask_transform, image_size, mask_root=None):
    with open(json_path, "r", encoding="utf-8") as file:
        data = json.load(file)

    caption = data.get("caption") or data.get("long_prompt")
    if not caption:
        raise KeyError(f"{json_path} must contain either 'caption' or 'long_prompt'.")
    ...
    for obj in data["object_list"]:
        mask_path = resolve_mask_path(obj["mask"], json_path, mask_root)
        if not mask_path.is_file():
            print(f"Skipping missing mask for {json_path.name}: {mask_path}")
            continue

        phrase = obj["prompt"]
        token_indices = obj.get("indices")
        if token_indices is None:
            ...
            token_indices = get_token_indices(offsets, start_char, end_char)

        masks.append(mask_transform(Image.open(mask_path).convert("L")))
    ...
    return {
        "prompts": [caption],
        "mask_list": torch.stack(masks).unsqueeze(0),
        "part_prompt": [phrases],
