...
def log_validation(flux_transformer, args, accelerator, weight_dtype, step, is_final_validation=False):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        ...
        for obj in data.get("object_list", []):
            ...
            # mask_path = os.path.join('/root/autodl-tmp/lisa_test_add',obj["mask"])
            mask_path = os.path.join('/root/autodl-tmp/lisa_test',obj["mask"])
    ...
    def preprocess_train(examples):
        ...
        for json_path in json_paths:
            ...
            for obj in data.get("object_list", []):
                ...
                if  'json_noun_all' in json_path:
                    mask_path_base = os.path.dirname(json_path).replace('json_noun_all','')
                else:
                    mask_path_base = os.path.dirname(json_path).replace('json_noun','')

                mask_path = os.path.join(mask_path_base,obj["mask"])
                # mask_path = os.path.join('/root/autodl-tmp/lisa_data_generation',obj["mask"])
                mask_img = Image.open(mask_path).convert("L")
                mask_tensor = mask_image_transforms(mask_img)  # Optional: customize mask_transforms.
                mask_list.append(mask_tensor)
                part_prompt.append(obj["prompt"])
...
def main(args):
    ...
    # for file in files:
    #     load_files_dict = load_file(os.path.join('/root/autodl-tmp/a_train_controlnet_4_lora__only_flxu_train/pose-control-lora_all_linear',"checkpoint-30000", "transformer", file))
            ...
            with accelerator.accumulate(flux_transformer):
                ...
                for bs in range(position_mask.shape[0]):
                    ...
                    for i in range(instance_num):
                        image_instance_text_idxs  = batch["indices"][bs][i]  
                ...
                joint_attention_kwargs['attention_mask'] = atten_mask_list
                ...
                model_pred = flux_transformer(
                    ...
                    atten_mask_list = atten_mask_list,
                    atten_mask_1_list = torch.zeros_like(atten_mask_list).bool(),
                )[0]
